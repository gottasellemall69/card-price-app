  ...
  const matchCards=() => {
    ...
    const matchedResults=cardData.data.filter((card) => {
      ...
      const cardSets=(card.card_sets||[]).map((set) => ({
        ...
        edition: set.set_edition? set.set_edition.toLowerCase():undefined,

      }));

      return userCardList.some((entry) => {
        const [name,numberOrSet,setEdition]=entry.split(',').map((item) => item.trim().toLowerCase());
        // Check if the name, set name, set code, or set edition of the card matches the user's input
        return (
          ...
          (numberOrSet==='set'&&
            cardSets.some((set) =>
              ...
              (setEdition&&set.edition&&set.edition.includes(setEdition))
              ))
        );
      });
    });
